...
defineFeature(feature, test => {
  test("When user hasn't specified a number. 32 is the default number", ({
    ...
    when(
      'a user does not enter or select a specific number of events to be shown',
      () => {}
    );
  ...
  test('User can change the number of events they want to see', ({
    ...
    then(
      'the list of events will be re-rendered, as a result displaying the number of events that the user has specified',
      () => {
        AppWrapper.update();
